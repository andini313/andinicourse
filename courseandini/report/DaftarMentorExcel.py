...
class PartnerXlsx(models.AbstractModel):
    _name = 'report.courseandini.report_mentor_xlsx'
    _inherit = 'report.report_xlsx.abstract'
    tgl_laporan = fields.Date.today()

    def generate_xlsx_report(self, workbook, data, mentor):
        # One sheet by partner
        sheet = workbook.add_worksheet('Daftar Mentor')
        # Menambahkan informasi tanggal laporan
        sheet.write(0, 0, str(self.tgl_laporan))
        sheet.write(1, 0, 'Nama Mentor')
        ...
        for obj in mentor:
            ...
            sheet.write(row, col, obj.name_mentor)
            sheet.write(row, col + 1, obj.alamat)
            sheet.write(row, col + 2, obj.no_tlp)
